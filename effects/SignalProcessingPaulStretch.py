...
class SignalProcessingPaulStretch:
    ...
    def process(
        ...
        if end_size < 16:
            end_size = 16
        fade_out = torch.linspace(1.0, 0.0, end_size, device=waveform.device, dtype=waveform.dtype)
        ...
        window = torch.pow(1.0 - torch.pow(torch.linspace(-1.0, 1.0, window_size, device=waveform.device, dtype=waveform.dtype), 2.0), 1.25)

        # Initialize old windowed buffer
        old_windowed_buf = torch.zeros((nchannels, window_size), device=waveform.device, dtype=waveform.dtype)
        ...
        while True:
            ...
            phases = torch.rand(freqs.shape, device=waveform.device, dtype=waveform.dtype) * 2 * math.pi
            ...
            output = buf_ifft[:, :half_window_size] + old_windowed_buf[:, half_window_size:]
...
if __name__ == "__main__":
    ...
    result = node.process(input,stretch_factor=stretch_factor, window_size_seconds=window_size_seconds)[0]
    ...
    torchaudio.save('ComfyUI_SignalProcessing/audio/tests/paulstretch.wav',combined.cpu(),sample_rate_audio)
