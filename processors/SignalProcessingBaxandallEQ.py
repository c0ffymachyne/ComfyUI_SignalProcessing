...
class SignalProcessingBaxandallEQ:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "audio_input": ("AUDIO",),
                "bass_gain_db": ("FLOAT", {"default": 1.0, "min": -20.0, "max": 20.0, "step": 0.1}),
                "treble_gain_db": ("FLOAT", {"default": 1.0, "min": -20.0, "max": 20.0, "step": 0.1}),
            },
        }
    ...
    def process(self, audio_input, bass_gain_db=0.0, treble_gain_db=0.0):
        ...
        b_bass, a_bass = self.design_rbj_shelf(sample_rate, freq=100.0, gain_db=bass_gain_db, shelf_type='low')
        waveform = torchaudio.functional.lfilter(waveform, a_bass.to(waveform.device), b_bass.to(waveform.device),clamp=False)

        # Apply Treble Shelf (high shelf) using RBJ formula
        b_treble, a_treble = self.design_rbj_shelf(sample_rate, freq=10000.0, gain_db=treble_gain_db, shelf_type='high')
        waveform = torchaudio.functional.lfilter(waveform, a_treble.to(waveform.device), b_treble.to(waveform.device),clamp=False)
    ...
    def design_rbj_shelf(self, sr, freq, gain_db, shelf_type='low'):
        ...
        alpha = math.sin(w0)/2.0 * math.sqrt((A + 1/A)*(1.0/1.0 - 1) + 2.0)  # S=1.0
...
class SignalProcessingBaxandall3BandEQ:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "audio_input": ("AUDIO",),
                "bass_gain_db": ("FLOAT", {"default": 1.0, "min": -20.0, "max": 20.0, "step": 0.1}),
                "mid_gain_db": ("FLOAT", {"default": 0.0, "min": -20.0, "max": 20.0, "step": 0.1}),
                "treble_gain_db": ("FLOAT", {"default": 1.0, "min": -20.0, "max": 20.0, "step": 0.1}),
                "low_freq": ("FLOAT", {"default": 100.0, "min": 20.0, "max": 500.0, "step": 1.0}),
                "mid_freq": ("FLOAT", {"default": 1000.0, "min": 200.0, "max": 5000.0, "step": 10.0}),
                "high_freq": ("FLOAT", {"default": 10000.0, "min": 2000.0, "max": 20000.0, "step": 100.0}),
                "mid_q": ("FLOAT", {"default": 0.7, "min": 0.1, "max": 10.0, "step": 0.1}),
            },
        }
    ...
    def process(
        ...
        # Low shelf filter
        b_low, a_low = self.design_rbj_shelf(sample_rate, low_freq, bass_gain_db, shelf_type='low')
        ...
        # High shelf filter
        b_high, a_high = self.design_rbj_shelf(sample_rate, high_freq, treble_gain_db, shelf_type='high')
...
if __name__ == "__main__":
    ...
    torchaudio.save('ComfyUI_SignalProcessing/audio/tests/baxandall_eq.wav', output_audio.cpu(), sample_rate_audio)
    ...
    result = node.process(input_tensors, bass_gain_db=4.0, mid_gain_db=-2.0, treble_gain_db=3.0, low_freq=100, mid_freq=1000, high_freq=10000, mid_q=0.7)[0]

    output_audio, out_sr = audio_from_comfy_2d(result)
    torchaudio.save('ComfyUI_SignalProcessing/audio/tests/3band_baxandall_eq_output.wav', output_audio.cpu(), out_sr)
